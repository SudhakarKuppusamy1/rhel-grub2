...
process_content (grub_uint8_t *content, grub_int32_t size, grub_pkcs7_data_t *msg)
{
  ...
  for (i = 0; i < signer_count; i++)
    {
      ...
      gcry_err = gcry_mpi_scan (&(msg->signers[i].sig_mpi), GCRYMPI_FMT_USG,
      ...
      /*
       * Use msg->signer_count to track fully populated signerInfos so we
 ...
 cleanup_signerInfos:
  for (i = 0; i < msg->signer_count; i++)
    gcry_mpi_release (msg->signers[i].sig_mpi);
