...
grub_x509_cert_parse (const void *data, grub_size_t data_size, grub_x509_cert_t *results)
{
 ...
 cleanup_mpis:
  gcry_mpi_release (results->mpis[GRUB_RSA_PK_MODULUS]);
  gcry_mpi_release (results->mpis[GRUB_RSA_PK_EXPONENT]);
 cleanup_name:
...
grub_x509_cert_release (grub_x509_cert_t *cert)
{
  ...
  gcry_mpi_release (cert->mpis[GRUB_RSA_PK_MODULUS]);
  gcry_mpi_release (cert->mpis[GRUB_RSA_PK_EXPONENT]);
}
